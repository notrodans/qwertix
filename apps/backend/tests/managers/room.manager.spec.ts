...
describe('Room Logic', () => {
	...
	it('should handle race flow', () => {
		...
		// Update progress
		room.updateProgress('socket-1', Math.floor(totalLength / 2));
		expect(room.participants().get('socket-1')?.progress).toBeGreaterThan(40);

		// Finish race
		room.updateProgress('socket-1', totalLength);
		expect(room.participants().get('socket-1')?.finishedAt).toBeDefined();
		expect(room.participants().get('socket-1')?.rank).toBe(1);

		room.updateProgress('socket-2', totalLength);
