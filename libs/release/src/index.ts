...
export class ReleaseManager {
  ...
  constructor(executor: ShellExecutor, options: ReleaseOptions) {
    ...
    this.isPreview = !!(options.preview || options['preview-fe'] || options['preview-be']);
  }

  // biome-ignore lint/suspicious/noExplicitAny: generic promise result
  private async runStep(name: string, action: () => Promise<any>, forceRun = false) {
    console.log(`\n📦 Step: ${name}`);
  ...
  public async run() {
    ...
    if (this.isDry) console.log(`[DRY RUN] No changes will be committed or pushed.`);

    // 1. Run Tests
    await this.runStep("Tests", async () => {
      if (this.isPreview) return;
      await this.$`bun run test:all`;
    }, true); // Always run tests even in dry run unless preview

    // 2. Preview or Generate Changelog
    if (this.isPreview) {
      await this.runStep("Preview Changelog", async () => {
        await this.$`git-cliff --unreleased --strip all`;
      }, true); // Always run preview
      return;
    }
    ...
    await this.runStep("Bump Version & Generate Changelog", async () => {
      ...
      const nextVersion = (await this.$`git-cliff --bumped-version`.text()).trim();
      if (!nextVersion) {
    ...
    await this.runStep("Commit and Tag", async () => {
      const version = (await this.$`node -p "require('./package.json').version"`).text().trim();
