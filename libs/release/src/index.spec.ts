...
describe("ReleaseManager", () => {
  ...
  beforeEach(() => {
    ...
    mockShell = mock((strings: TemplateStringsArray, ...values: unknown[]) => {
      const cmd = reconstructCmd([strings, ...values]);
      executedCommands.push(cmd);
      
      const getOutput = () => {
          if (cmd.includes("git-cliff --bumped-version")) return "1.2.3\n";
          if (cmd.includes("package.json") && cmd.includes("version")) return "1.2.3\n";
          return "";
      };
  ...
  it("should run dry run flow correctly", async () => {
    ...
    expect(executedCommands.some(c => c.includes("git-cliff --bump"))).toBe(false);
  });
  ...
  it("should run full release flow correctly", async () => {
    ...
    expect(executedCommands.some(c => c.startsWith("npm version 1.2.3"))).toBe(true);
    expect(executedCommands).toContain("git add package.json CHANGELOG.md");
    expect(executedCommands).toContain('git commit -m "chore(release): prepare for v1.2.3"');
  ...
  it("should add [no-deploy] flag when requested", async () => {
    ...
    expect(executedCommands).toContain('git commit -m "chore(release): prepare for v1.2.3 [no-deploy]"');
  });
